...
static void event_handler(lv_obj_t * obj, lv_event_t event)
{
    ...
    if (event == LV_EVENT_FOCUSED) {
        lv_label_set_text(top_bar_str, "Buzzer settings");
...
static void accept_event_handler(lv_obj_t * obj, lv_event_t event)
{
    switch (event) {
        ...
        case LV_EVENT_SHORT_CLICKED: {
            ...
            eeprom_data->buzzerState = (uint32_t) lv_checkbox_is_checked(checkbox);
...
void gui_setup_buzzer(void)
{
    ...
    top_bar = lv_obj_create(gui_screen.setup_buzzer_scr, NULL);
    ...
    top_bar_str = lv_label_create(gui_screen.setup_buzzer_scr, NULL);
    lv_label_set_text(top_bar_str, "Buzzer settings");
    lv_obj_set_style_local_text_color(top_bar_str, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, LV_COLOR_WHITE);
    lv_obj_align_origo(top_bar_str, top_bar, LV_ALIGN_CENTER, 0, 0);

    checkbox = lv_checkbox_create(gui_screen.setup_buzzer_scr, NULL);
    if (eeprom_data->buzzerState) {
...
void gui_setup_buzzer_enable_focus(void)
{
