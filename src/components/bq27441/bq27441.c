...
bool BQ27441_init(void) {
    if (HAL_I2C_IsDeviceReady(&hi2c1, (BQ72441_I2C_ADDRESS << 1), 5, 50) != HAL_OK)
        return false;
    else
        return true;
...
#else
    HAL_I2C_Mem_Read(&hi2c1, (BQ72441_I2C_ADDRESS << 1), subAddress, 1, dest, count, 50);
#endif
...
#else
    HAL_I2C_Mem_Write(&hi2c1, (BQ72441_I2C_ADDRESS << 1), subAddress, 1, src, count, 50);
#endif
    return true;
}
